	...
	tests := map[string]struct {
		...
		"search and find single package - echo when installed version is less than available version": {
			...
			init: func(m *mocked) {
				...
				m.term.On("Printf", bold.Sprintf("  Installed Version:")+" %s\n", []interface{}{"1.0.0"}).Return().Once()
